...
void merge(int array[], int const left, int const mid, int const right)
{
    ...
    while (indexOfSubarrayOne < sizeOFSubArrayOne && indexOfSubarrayTwo < sizeOFSubArrayTwo)
    {
        ...
        {                                                                // if element of subarray 1 at that idx is smaller than ele which is there at same idx in subarray 2
...
{
    if (start >= end)
        return; // if input array is empty or have just one element
